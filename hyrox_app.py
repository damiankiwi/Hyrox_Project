...
def analyze_my_result():
    ...
    try:
        my_times = {col: pd.to_timedelta(entry.get()).total_seconds() for col, entry in
                    zip(time_cols[1:], [work_entry, rox_entry, run_entry])}
        my_times["total_time"] = pd.to_timedelta(total_var.get()).total_seconds()
    except:
        analysis_label.config(text="Niepoprawny format czasu")
    ...
    for col in time_cols:
        ...
        analysis_text += f"{entry_label(col)}: miejsce {place} na {total_athletes} osób\n"
        ...
        if diff_sec > 0:
            ...
            improvement_text += f"Aby być 1 w {entry_label(col)}, popraw o {diff_min}:{diff_sec_rem:02d} min\n"
        else:
            improvement_text += f"Jesteś najlepszy w {entry_label(col)}!\n"
...
# --- GUI ---
root = tk.Tk()
root.title("Analiza zawodników")

# Filtry
filter_frame = tk.Frame(root)
filter_frame.pack(pady=10)

gender_var = tk.StringVar(value="wszystkie")
tk.Label(filter_frame, text="Płeć:").grid(row=0, column=0, padx=5)
gender_dropdown = ttk.Combobox(filter_frame, textvariable=gender_var, values=["wszystkie", "male", "female"],
                               state="readonly", width=10)
gender_dropdown.grid(row=0, column=1, padx=5)

age_var = tk.StringVar(value="wszystkie")
age_options = ["wszystkie"] + sorted(df["age_group"].unique())
tk.Label(filter_frame, text="Wiek:").grid(row=0, column=2, padx=5)
age_dropdown = ttk.Combobox(filter_frame, textvariable=age_var, values=age_options, state="readonly", width=10)
age_dropdown.grid(row=0, column=3, padx=5)

division_var = tk.StringVar(value="wszystkie")
tk.Label(filter_frame, text="Dywizja:").grid(row=0, column=4, padx=5)
division_dropdown = ttk.Combobox(filter_frame, textvariable=division_var, values=["wszystkie", "open", "pro"],
                                 state="readonly", width=10)
division_dropdown.grid(row=0, column=5, padx=5)

filter_button = tk.Button(filter_frame, text="Filtruj", command=filter_data)
...
tk.Label(input_frame, text="Twój total_time:").grid(row=0, column=0, padx=5)
total_entry = tk.Entry(input_frame, textvariable=total_var, width=10, state="readonly")
total_entry.grid(row=0, column=1, padx=5)

tk.Label(input_frame, text="Twój work_time:").grid(row=0, column=2, padx=5)
...
tk.Label(input_frame, text="Twój roxzone_time:").grid(row=0, column=4, padx=5)
...
tk.Label(input_frame, text="Twój run_time:").grid(row=0, column=6, padx=5)
...
analyze_button = tk.Button(input_frame, text="Analizuj", command=analyze_my_result)
...
update_total()  # obliczenie total_time przy starcie

# Tabela
columns = ["event_name", "gender", "nationality", "age_group", "division", "total_time", "work_time", "roxzone_time",
           "run_time"]
